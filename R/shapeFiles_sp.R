...
#' @importFrom sp CRS
readSingleShapeFile_sp <- function(fn, proj4string = sp::CRS(as.character(NA)), 
...
# Example: shape_file_data <- readShapeFiles_sp(GWSDAT_Options$ShapeFileNames)
readShapeFiles_sp <- function(ShapeFileNames) {
  
  ShapeFiles <- NULL
  
  if (length(ShapeFileNames) > 0) {
    ShapeFiles <- list()
    mycount <- 0
    
    for (i in 1:length(ShapeFileNames)) {
      
      tempShapeFile <- try(readSingleShapeFile_sp(ShapeFileNames[i]))
      
      
      
      if (inherits(tempShapeFile, "try-error")) {
        
        msg <- paste("Warning: Error Inputting '", ShapeFileNames[i],"' shapefile", sep = "")
        showNotification(msg, type = "warning", duration = 20) 
        
        if (length( grep(".shp",tolower(ShapeFileNames[i]))) == 0) {
        ...
        if (file.exists(ShapeFileNames[i]) != TRUE) {
          msg <- paste("Warning: File '", ShapeFileNames[i],"' does not exist.", sep = "")
          showNotification(msg, type = "warning", duration = 20) 
        }
